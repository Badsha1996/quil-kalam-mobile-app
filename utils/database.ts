...
// Neon database for published works (configure with your connection string)
  ...
  try {
    ...
    const publishedProject = await neonDb`
      INSERT INTO published_projects (
        ...
        ${settings?.publication_date}, ${settings?.price}, ${settings?.language || "en"},
        ${settings?.copyright_text}, ${settings?.categories ? settings.categories.split(",") : []},
...
export * from "./database"; // Import old functions
